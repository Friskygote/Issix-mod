...
func _run():
	...
	if state == "confessionary_med3":
		saynn("Master Issix puts a tiny viol inside of the container of mechanical arm before the enclosure seals itself shut. There are loud whirling sounds")
	...
	if state == "confessionary_soul_wakeup":
		...
		addButton("Corner", "COme back at the corner", "confessionary_soul_end")

	if state == "confessionary_soul_end":
...
func _react(_action: String, _args):
	...
	if _action == "confessionary_soul_end":
